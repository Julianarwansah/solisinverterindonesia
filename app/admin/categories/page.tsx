...
export default async function AdminCategoriesPage() {
    ...
    return (
        <div className="space-y-12">
            {/* Action Header */}
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 bg-white p-12 rounded-[50px] shadow-sm border border-gray-100">
                <div>
                    <h2 className="text-[10px] font-black text-orange-500 uppercase tracking-[0.3em] mb-2">Inventory System</h2>
                    <h1 className="text-4xl font-[1000] text-gray-950 tracking-tight">Kategori Produk</h1>
                </div>
                <Link
                    href="/admin/categories/new"
                    className="inline-flex items-center justify-center gap-4 px-10 py-6 bg-gray-950 text-white font-black rounded-[32px] hover:bg-orange-600 transition-all hover:shadow-2xl hover:shadow-orange-600/20 active:scale-95"
                >
                            ...
                            {categories.length === 0 ? (
                                ...
                                categories.map((cat) => (
                                    <tr key={cat.id} className="hover:bg-[#F8F9FD]/50 transition-colors group">
                                        ...
                                        <td className="px-12 py-8 text-gray-400 font-bold font-mono text-sm tracking-tighter">
                                            /{cat.slug}
                                        </td>
                                        <td className="px-12 py-8">
                                            <p className="text-sm font-medium text-gray-500 line-clamp-1 max-w-xs">{cat.description || '-'}</p>
                                        </td>
                                        <td className="px-12 py-8 text-right">
                                            <div className="flex items-center justify-end gap-3 translate-x-4 opacity-0 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-300">
                                                <Link
                                                    href={`/admin/categories/${cat.id}`}
                                                    className="p-4 bg-white border border-gray-200 text-gray-400 hover:text-orange-600 hover:border-orange-200 rounded-2xl transition-all shadow-sm active:scale-9"
                                                >
