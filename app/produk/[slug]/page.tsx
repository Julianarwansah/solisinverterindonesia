...
export async function generateStaticParams() {
    console.log('Generating static params for products via Laravel...');
    try {
        const products = await laravel.products.list();
        console.log(`Found ${products.length} products from Laravel`);
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    try {
        const product = await laravel.products.get(slug);
...
async function getProduct(slug: string) {
    try {
        return await laravel.products.get(slug);
    } catch (error) {
...
export default async function ProductDetail({ params }: Props) {
    ...
    if (product.images && product.images.length > 0) {
        ...
        const url = featuredImg.startsWith('http') ? featuredImg : `${baseUrl}/storage/${featuredImg}`;
        images.push(url);
    }
    ...
    if (product.images && product.images.length > 1) {
        product.images.slice(1).forEach((img: string) => {
            const url = img.startsWith('http') ? img : `${baseUrl}/storage/${img}`;
                    ...
                    <div className="space-y-8">
                        ...
                        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-xl shadow-gray-100/50 space-y-6">
                            <div>
                                ...
                                <p className="text-gray-500 text-sm">Tim ahli kami siap membantu Anda memilih solusi inverter terbaik.</p>
                            </div>
