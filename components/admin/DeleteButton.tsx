...
export default function DeleteButton({ id, type }: DeleteButtonProps) {
    ...
    return (
        <button
            ...
            disabled={loading}
            className={`p-3 bg-white border border-gray-100 text-gray-400 hover:text-red-600 hover:border-red-100 rounded-xl transition-all shadow-sm active:scale-90 ${loading ? 'opacity-50 cursor-not-allowed' : ''}`}
            title="Hapus"
        >
            {loading ? (
